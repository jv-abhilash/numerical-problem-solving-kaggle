# src/p2_routing/classification/router.py
from __future__ import annotations
from typing import List, Dict, Tuple, Optional, cast

from src.shared.schema import Question
from src.storage.interfaces import RouteRecord, Topic, Difficulty
from .classifier import classify_topic, assess_difficulty, infer_tools
from src.p2_routing.interfaces import RouterEngine

# Keep this file aligned with src/storage/interfaces.py
_ALLOWED_TOPICS: set[str] = {"algebra", "calculus", "discrete", "geometry"}

def _coerce_topic(raw: Optional[str]) -> Topic:
    s = (raw or "").strip().lower()

    # map synonyms / fine-grained to the 4 umbrellas
    if s in {"trig", "trigonometry", "integration", "differentiation", "limits", "calculus"}:
        canon = "calculus"
    elif s in {"stats", "stat", "prob", "probability", "sets", "lpp", "discrete"}:
        canon = "discrete"
    elif s in {"vectors", "complex", "algebra"}:
        canon = "algebra"
    elif s in {"geometry"}:
        canon = "geometry"
    else:
        # default because "mixed" was removed from your schema
        canon = "algebra"

    # safety: ensure we only return allowed Topic
    if canon not in _ALLOWED_TOPICS:
        canon = "algebra"
    return cast(Topic, canon)
...
def _route_one(q: Question) -> RouteRecord:
    stem = q["stem"]
    opts = q.get("opts", [])
    topic_raw, subtopic, conf = classify_topic(stem)
    diff_raw = assess_difficulty(stem, opts)
    tools = infer_tools(stem)

    topic = _coerce_topic(topic_raw)
    diff = _coerce_difficulty(diff_raw)
    bucket = f"{topic}:{diff}"
...
def route_questions(questions: List[Question], batch_size: int = 20) -> Tuple[List[RouteRecord], Dict[str, int]]:
...
class HeuristicRouterEngine(RouterEngine):
    """Adapter so routing can be swapped for MCP without changing call sites."""
    def route_batch(self, questions: List[Question]) -> List[RouteRecord]:
        routes, _ = route_questions(questions, batch_size=len(questions))
        return routes
