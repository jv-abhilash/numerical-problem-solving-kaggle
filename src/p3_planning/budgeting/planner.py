...
def build_steps(q: Question, route: RouteRecord) -> List[str]:
    topic = route["topic"]
    steps: List[str] = []

    if topic == "calculus":
        steps = [
            "Parse the function/limit/integral from the stem.",
            "Identify the calculus technique (e.g., substitution, product rule, L'Hôpital).",
            "Carry out symbolic steps and simplify.",
            "Compare result with given options and select the closest match.",
        ]
    elif topic == "algebra":
        steps = [
            "Extract variables/expressions and constraints.",
            "Apply algebraic manipulation (factorization, identities, solving).",
            "Simplify to canonical form.",
            "Match with options.",
        ]
    elif topic == "geometry":
        steps = [
            "Translate geometry data to analytic form (coordinates/lengths/angles).",
            "Apply known theorems/formulae.",
            "Compute required quantity.",
            "Validate with options.",
        ]
    elif topic == "discrete":
        steps = [
            "Determine sub-area (probability/sets/combinatorics/matrix).",
            "Apply counting rules or set/probability identities as needed.",
            "Evaluate numerical expression.",
            "Select the option that matches.",
        ]
    else:
        # fallback (shouldn't happen with 4-topic taxonomy)
        steps = [
            "Extract key quantities.",
            "Apply general mathematical reasoning.",
            "Compute/derive target value.",
